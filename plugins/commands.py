...
async def start(client:Client, message):
        ...
        try: 
            ...
            if refData and refData.split("-", 1)[0] == "Jisshu":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgIAAxkBAAKa2Geu-eGBdi4pUfyvu8YZjV7I7Iv8AAJbKAACJP94SyaXUlooV0sfNgQ") 
    ...
    if len(message.command) != 2:
        buttons = [[
                            InlineKeyboardButton('☆ Tʀᴇɴᴅɪɴɢ ᴏɴ ʙᴏᴛ ☆', callback_data="trending')
                        ],[
                            InlineKeyboardButton("Hᴇʟᴘ ⚙️", callback_data='admincmd'),
                            InlineKeyboardButton('Rᴇғᴇʀ ⚜️', callback_data="reffff")
                        ],[
...
async def refer(bot, message):
    ...
    m=await message.reply_sticker("CAACAgIAAxkBAAKa22eu-1LHXSnJJDH4pU7sBl5KYSQkAAKLKQACvFuBS6nUeJX6olesNgQ")      
    await m.delete()
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
